...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex flex-col">
      <header className="sticky top-0 z-50 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white py-4 px-4 shadow-lg border-b border-slate-700/50 backdrop-blur-sm">
        <div className={`${isTop ? 'max-w-4xl mx-auto flex items-center gap-3 transition-all duration-300' : 'max-w-4xl mx-auto hidden transition-all duration-300'}`}>
          <Shield size={30} className="text-red-400 shrink-0" />
          <div>
        ...
        <nav className="mt-2 border-t border-slate-800/30 transition-all duration-200">
          <div className="max-w-4xl mx-auto flex gap-2 flex-wrap py-2 justify-center sm:justify-start">
            {sections.map(s => (
              <button
                key={s.key}
                onClick={() => setActive(s.key)}
                className={`text-sm px-3 py-1.5 rounded-md focus:outline-none transition-all duration-200 border border-transparent ${active === s.key ? 'bg-white text-slate-900 shadow-sm scale-[1.01]' : 'text-slate-200 hover:bg-slate-800/70 hover:text-white hover:border-slate-700 hover:shadow-md hover:-translate-y-0.5 hover:scale-[1.02]'}`}>
                {s.label}
              </button>
            ))}
          </div>
      ...
      <main ref={mainRef} className="flex-1 max-w-5xl mx-auto px-6 py-12 w-full">
        <div className={`bg-white rounded-xl shadow-lg border border-slate-200 p-8 transition-all duration-300 ${isTransitioning ? 'opacity-60 translate-y-2' : 'opacity-100 translate-y-0'}`}>
          <ActiveComponent />
        </div>
      </main>

      <footer className="bg-slate-100 text-slate-600 text-sm py-4 px-6">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center sm:items-center justify-between gap-3">
          ...
          <a
            ...
            className={`inline-flex items-center gap-2 bg-slate-900 text-white text-sm px-3 py-1.5 rounded-md transition-all shadow-sm ${contactHover ? 'scale-105 -translate-y-0.5 -rotate-2 shadow-lg' : 'hover:scale-105 hover:-translate-y-0.5'}`}>
